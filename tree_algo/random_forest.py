...
class RandomForestClassifier(object):
    ...
    def __init_bootstrap_kernel(self):
        ...
        self.bootstrap_fill= mk_kernel((ctype_indices,), "bootstrap_fill","bootstrap_fill.cu")
        self.bootstrap_reshuffle, tex_ref = mk_tex_kernel((ctype_indices, 128), "bootstrap_reshuffle","tex_mark", "bootstrap_reshuffle.cu")
    ...
    def _get_sorted_indices(self, sorted_indices):
        ...
        if not self.bootstrap:
          ...
          self.bootstrap_fill.prepared_call((1, 1),(512, 1, 1),random_sample_idx_gpu.ptr,self.mark_table.ptr,n_samples,self.stride)

          self.bootstrap_reshuffle.prepared_call((self.n_features, 1),(128, 1, 1),self.mark_table.ptr,sorted_indices_gpu_original.ptr,sorted_indices_gpu.ptr,self.stride)
    ...
    def fit(self, samples, target, bfs_threshold = None):
        ...
        if self.verbose: 
          print ("bsf_threadshold : %d; bootstrap : %r; min_samples_split : %d" % (self.bfs_threshold,self.bootstrap,  self.min_samples_split))
          print ("n_samples : %d; n_features : %d; n_labels : %d; max_features : %d" % (self.stride, self.n_features, self.n_labels, self.max_features))
...
def predict(self, x):
    ...
    def __compile_kernels(self):
        ...
        dfs_module = compile_module("dfs_module.cu", (n_threads, n_shf_threads, n_labels, ctype_samples, ctype_labels, ctype_counts, ctype_indices, self.MAX_BLOCK_PER_FEATURE, self.debug))
        ...
        bfs_module = compile_module("bfs_module.cu", (self.BFS_THREADS, n_labels, ctype_samples, ctype_labels, ctype_counts, ctype_indices,  self.debug))
        ...
        self.predict_kernel = mk_kernel(params = (ctype_indices, ctype_samples, ctype_labels),func_name = "predict",kernel_file = "predict.cu",prepare_args = "PPPPPPPii")
